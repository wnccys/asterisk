...
impl Scanner {
    pub fn scan_token(&mut self, chars: &[char]) -> Token {
        self.start = self.current;
        if self.reach_source_end(chars) {
        ...
        if chars[self.current - 1].is_ascii_digit() {
    ...
    fn alphanumeric(&mut self, chars: &[char]) -> Token {
    ...
    fn identifier(&mut self, chars: &[char]) -> TokenCode {
    ...
    fn check_keyword(
        ...
        while matched_chars < matcher.len()
            ...
            && matcher.chars().nth(matched_chars).unwrap()
                == chars[self.start + matcher_start + matched_chars]
        {
            matched_chars += 1;
        }
    ...
    fn number(&mut self, chars: &[char]) -> Token {
        while !self.reach_source_end(chars) && chars[self.current].is_ascii_digit() {
            self.current += 1;
        }

        if !self.reach_source_end(chars) && chars[self.current] == '.' {
            self.current += 1;
            if !self.reach_source_end(chars) && chars[self.current].is_ascii_digit() {
                while !self.reach_source_end(chars) && chars[self.current].is_ascii_digit() {
                    self.current += 1
                }
            }
    ...
    fn string(&mut self, chars: &[char]) -> Token {
    ...
    fn skip_comment(&mut self, chars: &[char]) -> Token {
        while !self.reach_source_end(chars) && chars[self.current] != '\n' {
            self.current += 1;
        }
    ...
    fn reach_source_end(&self, chars: &[char]) -> bool {
